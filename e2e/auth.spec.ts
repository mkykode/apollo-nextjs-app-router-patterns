...
test("signing in sets the session cookie, lands on the callback, and renders the user on the server", async ({
  ...
  // Signed in, the login page itself sends the visitor on to /account. On this branch the
  // header is a plain link, so there is no user menu to assert: reading the session there
  // would make every route dynamic, which is the whole point of the comparison.
  await page.goto("/login");
