...
test("an unknown track id is a real 404 when the page awaits the query before rendering", async ({
  page,
}) => {
  // /revalidate/track/[trackId] awaits GetTrack in the page itself, so notFound() throws
  // before any HTML is sent and the response status is a genuine 404.
  const response = await page.goto("/revalidate/track/does-not-exist");
