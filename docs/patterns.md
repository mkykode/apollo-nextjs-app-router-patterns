...
| `/cached`     | `query()` inside `"use cache"` (`cacheLife`, `cacheTag`) | On the server, only on a cache miss           | Finished markup, prerendered      | Empty                               | Server Action + `updateTag`          |
...
**Caching layers.** Apollo's `InMemoryCache` is per instance (per request on the server, per tab in the browser). Next.js adds the fetch Data Cache on top: `HttpLink({ fetchOptions: { next: { revalidate: 60 } } })` or `context.fetchOptions` per query. This branch runs Cache Components (`cacheComponents: true`): nothing is cached unless marked `"use cache"`, every route gets a prerendered shell, and dynamic data streams in under Suspense. Three things had to change from the classic model on `nextjs-app-router`: the route segment configs are gone (they are build errors); `/suspense` and `/background` gained a layout that awaits `connection()`, because their SSR fetch goes through the browser-side Apollo link that Next.js cannot observe and would otherwise be baked in at build; and the `PreloadQuery` pages await `connection()` because the integration mints a queryRef id with `crypto.randomUUID()`, which prerendering rejects. `/cached` moved from `next.revalidate` on the fetch to `"use cache"` + `cacheLife("minutes")` + `cacheTag`, invalidated by `updateTag` in the Server Action. The header wraps `PatternNav` in Suspense because `usePathname()` is runtime data inside the shared shell.
