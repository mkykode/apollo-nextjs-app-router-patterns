...
| `/revalidate`     | `query()` + Next.js Data Cache (`revalidate`, tags) | On the server, at most once a minute per query | Finished markup, prerendered      | Empty                               | Server Action + `updateTag`          |
...
**Activity.** `src/components/activity-tabs.tsx` hides the inactive tab with `<Activity mode="hidden">` instead of unmounting it, and a checkbox switches back to `{active && <Panel />}` so the difference is visible. Two things follow from the switch. The draft note is an uncontrolled `<textarea>`, so its text is DOM state and nothing in React holds it: unmounted it is gone, hidden it is still in the document. And hidden children still render, at a lower priority, so the quick look's `useSuspenseQuery` is already in flight before its tab is clicked; the Suspense boundary sits above both Activities and a hidden one that suspends does not trip it, which is what makes that pre-render invisible. It is `useBackgroundQuery`'s goal reached from the other end: the hook warms one known query, the boundary warms a whole subtree. What it does not reliably do is put the quick look back into a loading state when unmounted, because Apollo holds a resolved query ref in its suspense cache for a while after unmount; the dependable difference is structural, and that is what `e2e/activity.spec.ts` asserts. On the `use-cache` branch Cache Components already applies Activity at the route level, so page state survives back and forward; this component uses Activity directly and behaves the same on both branches.

**View transitions.** React's `<ViewTransition>` (a plain export of `react` in Next.js, which bundles the canary channel; the npm package still hides it, so `src/types/react-canary.d.ts` adds the types and `vitest.setup.ts` shims it) is activated by Transitions, Suspense, and `useDeferredValue`, and App Router navigations are transitions. Four patterns: the card and detail covers share a `name` per track and morph (`src/components/track-card.tsx`, `track-detail.tsx`); the RSC detail page's skeletons exit and its sections enter (`src/app/rsc/track/[trackId]/page.tsx`); `router.push(href, { transitionTypes })` on the card and `<Link transitionTypes>` on the back link tag navigations as forward or back, and `src/components/page-transition.tsx` maps the types to slide classes on each participating page (pages, never layouts, which persist); the client search keys its results by the deferred query so they crossfade. The morph only pairs when the destination renders in the navigation's commit: prefetched static pages (`/revalidate`) do, streamed pages (`/rsc`) and suspending queries (`/suspense`, whose `GetTrack` needs fields the list did not fetch) do not. The header carries `viewTransitionName: "site-header"` and is pinned in CSS. `e2e/view-transitions.spec.ts` records `document.startViewTransition` calls and catches the running animation names with stretched durations.
...
**Link chain.** Both clients use `createLinkChain`: `ErrorLink` observes, `RetryLink` retries transient failures on queries only (never mutations, GraphQL errors, or 4xx), `SetContextLink` merges the client's fixed headers with whatever the operation carried in its own `context.headers`, `HttpLink` terminates. That per-operation slot is the only way auth enters: the RSC client cannot read cookies on every operation without dragging every cached and static route that shares it into dynamic rendering, so the Server Action attaches the API token at the call site instead. The browser client sets no custom headers because each would need CORS approval, and it has no token to send.

**Authentication.** Better Auth with email and password, and sessions as rows in SQLite (`node:sqlite`, so no dependency and no native build); the Odyssey API ignores the token, so everything enforced is enforced by this app. Three checks, in order of trust: `src/proxy.ts` on `/account` only calls `getSessionCookie`, which looks for the cookie without verifying it or touching the database, and hands the decision to the pure `proxyRedirect` in `src/lib/auth/paths.ts` (optimistic, cheap, runs on prefetches, and safe to put in a file the Next.js docs say may be deployed to a CDN); it deliberately does not bounce cookie-holders off `/login`, because presence is not validity and a cookie whose row is gone would ping-pong between the two pages forever, so the login page makes that call itself with a verified session; the `/account` page calls `getSession()` (`src/lib/auth/session.ts`, the one wrapper over `auth.api.getSession` and `headers()`) again next to the data it renders, which does verify the signature and load the row, so a revoked session fails there even though it passed the proxy; and `registerView` calls it before the mutation and returns state without a session (every `"use server"` export is public). `src/instrumentation.ts` runs `prepareAuthDatabase()` once before the first request to create the schema and seed the demo user; the migration adds missing columns as well as missing tables, which is why `validateSchema` is off (a check that runs before that function could only fire spuriously, as it would on every `next build`). The Server Actions call `auth.api.signInEmail` and `auth.api.signOut` in-process rather than posting to `/api/auth`; the `nextCookies()` plugin is what lands the `Set-Cookie`, since a Server Action cannot set one by returning a header. Wrong credentials arrive as an `APIError` with status `UNAUTHORIZED`, turned into state; the login schema runs in the browser and again in the action; a `redirectTo` that is not a relative path is dropped. The header shows an **Account** link, not the user, because a cookie read in the root layout would make every route dynamic on this branch; the `use-cache` branch streams a user menu into the static shell instead. Better Auth throws at startup if `BETTER_AUTH_SECRET` is missing, so a misconfigured deployment does not boot.

**The API token, and why the session shape is a public API.** `registerView` needs a bearer token for the upstream API, and the obvious place to put it is the session object. Under Auth.js that was a trap worth keeping in the notes: one `session` callback fed both `auth()` and the public `GET /api/auth/session`, with no way to tell them apart, so a field added for a Server Action was published to the browser in the same move, and the HttpOnly cookie protected a session whose own endpoint gave away the credential derived from it. Better Auth can say what Auth.js could not. `accessToken` is a field on the session table declared `input: false, returned: false` (`src/lib/auth/auth.ts`): never accepted from a request body, never included in a response body. It is minted per sign-in by the `session.create.before` database hook, the successor to the Auth.js `jwt` callback. Because `returned: false` also hides it from `auth.api.getSession()`, reading it is a deliberate second step through `readAccessToken` in `src/lib/auth/access-token.ts`, which queries the row by session token; the token is therefore never part of a session payload in any code path. Note that `customSession`, Better Auth's plugin for reshaping the session response, is the same shape as the old Auth.js callback and would reintroduce the leak exactly. `e2e/auth.spec.ts` asserts the endpoint still identifies the user and still carries no token; flipping `returned` to `true` fails that test and nothing else, which is how quietly the original bug hid.
...
- Better Auth pushes a session's expiry out once `updateAge` is reached, so a page that reads the session can re-set the cookie from a response still in flight. The rollback test in `e2e/forms.spec.ts` waits for the stream to finish before clearing the cookie, and asserts the session is gone before submitting.
- `src/lib/auth/db.ts` opens a `node:sqlite` handle at import time, so a unit test that reaches it would create a database as a side effect. Tests mock the module they actually need instead: `@/lib/auth/session` for a session, `@/lib/auth/access-token` for the token, `@/lib/auth/auth` for `auth.api.signInEmail`.
